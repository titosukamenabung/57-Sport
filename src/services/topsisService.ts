...
type FilterName = (typeof REMOVAL_ORDER)[number];
 ...
 */
function buildWhere(
  prefs: MotorPreferences | undefined,
  excludeFilters: Set<FilterName>
): { where: any; activeFilters: FilterName[] } {
  const where: any = {};
  const activeFilters: FilterName[] = [];

  if (!prefs) return { where, activeFilters };
  ...
  if (!excludeFilters.has('harga') && (prefs.hargaMin !== undefined || prefs.hargaMax !== undefined)) {
    where.harga = {};
    if (prefs.hargaMin !== undefined) where.harga.gte = prefs.hargaMin;
    if (prefs.hargaMax !== undefined) where.harga.lte = prefs.hargaMax;
    activeFilters.push('harga');
  }

  if (!excludeFilters.has('cc') && (prefs.ccMin !== undefined || prefs.ccMax !== undefined)) {
    where.cc = {};
    if (prefs.ccMin !== undefined) where.cc.gte = prefs.ccMin;
    if (prefs.ccMax !== undefined) where.cc.lte = prefs.ccMax;
    activeFilters.push('cc');
  }

  if (!excludeFilters.has('tahunMotor') && (prefs.tahunMotorMin !== undefined || prefs.tahunMotorMax !== undefined)) {
    where.tahunMotor = {};
    if (prefs.tahunMotorMin !== undefined) where.tahunMotor.gte = prefs.tahunMotorMin;
    if (prefs.tahunMotorMax !== undefined) where.tahunMotor.lte = prefs.tahunMotorMax;
  ...
  if (!excludeFilters.has('efisiensiBbm') && (prefs.efisiensiBbmMin !== undefined || prefs.efisiensiBbmMax !== undefined)) {
    where.efisiensiBbm = {};
    if (prefs.efisiensiBbmMin !== undefined) where.efisiensiBbm.gte = prefs.efisiensiBbmMin;
    if (prefs.efisiensiBbmMax !== undefined) where.efisiensiBbm.lte = prefs.efisiensiBbmMax;
  ...
  if (!excludeFilters.has('masaPajakStnk') && (prefs.masaPajakStnkMin !== undefined || prefs.masaPajakStnkMax !== undefined)) {
    where.masaPajakStnk = {};
    if (prefs.masaPajakStnkMin !== undefined) where.masaPajakStnk.gte = prefs.masaPajakStnkMin;
    if (prefs.masaPajakStnkMax !== undefined) where.masaPajakStnk.lte = prefs.masaPajakStnkMax;
  ...
  if (!excludeFilters.has('kondisiFisik') && (prefs.kondisiFisikMin !== undefined || prefs.kondisiFisikMax !== undefined)) {
    where.kondisiFisik = {};
    if (prefs.kondisiFisikMin !== undefined) where.kondisiFisik.gte = prefs.kondisiFisikMin;
    if (prefs.kondisiFisikMax !== undefined) where.kondisiFisik.lte = prefs.kondisiFisikMax;
...
export const calculateTOPSIS = async (requestId: number, preferences?: MotorPreferences) => {
  try {
    // ======================================
    ...
    const excluded = new Set<FilterName>();
    const removedFilters: FilterName[] = [];
    let motors: MotorWithMerk[] = [];
    ...
    while (true) {
      ...
      if (motors.length >= 3) break;

      // Cari filter yang masih aktif dan belum dilepas (paling tidak penting dulu)
      const nextRemovable = REMOVAL_ORDER.find(
        (f) => !excluded.has(f) && activeFilters.includes(f)
      );
    ...
    let ranking: { id: number; namaMotor: string; skor: number; ranking: number }[];
...
export const computeTOPSISRanking = (
  motors: MotorWithMerk[],
  bobot: BobotInput,
  kriterias: { kode: string; atribut: string }[]
) => {
  // ======================================
...
export const calculateManualTOPSIS = async (
  motorIds: number[],
  weights: { kriteriaId: number; bobot: number }[]
) => {
  ...
  const weightMap = Object.fromEntries(
    weights.map((w) => [w.kriteriaId, w.bobot])
  );

  const bobot: BobotInput = {
...
// ======================================
// Full TOPSIS computation (returns all intermediate data)
// ======================================

interface FullTopsisData {
  decisionMatrix: any[];
  pembagi: any;
...
function computeTOPSISFullData(
  motors: MotorWithMerk[],
  bobot: BobotInput,
  kriterias: { kode: string; atribut: string }[]
): FullTopsisData {
